...
{
    [InitializeOnLoad]
    public class TheBlackArms_ImportPanel_Prefabs : EditorWindow
    {
       private const string Url = "https://github.com/TheBlackArms/TBAUDK/";
        ...
        [MenuItem("TheBlackArms/Import panel/Prefabs", false, 501)]
        public static void OpenImportPanel()
        {
            GetWindow<TheBlackArms_ImportPanel_Prefabs>(true);
        }

        public void OnEnable()
        {
            titleContent = new GUIContent("TheBlackArms Import panel prefabs");

            TheBlackArms_ImportManager_Prefabs.checkForConfigUpdate();
            LoadJson();
        ...
        public static void LoadJson()
        {
            ...
            dynamic configJson =
                JObject.Parse(File.ReadAllText(TheBlackArms_Settings.projectConfigPath + TheBlackArms_ImportManager_Prefabs.configName));

            Debug.Log("Server Asset Url is: " + configJson["config"]["serverUrl"]);
            TheBlackArms_ImportManager_Prefabs.serverUrl = configJson["config"]["serverUrl"].ToString();
            ...
            foreach (var asset in assets)
            {
                ...
                else
                {
                    if (GUILayout.Button(
                        ...
                        " " + asset.Key))
                    {
                        TheBlackArms_ImportManager_Prefabs.downloadAndImportAssetFromServer(asset.Value);
                    }

                    if (GUILayout.Button("Del", GUILayout.Width(40)))
                    {
                        TheBlackArms_ImportManager_Prefabs.deleteAsset(asset.Value);
                    }
                }
                GUILayout.EndHorizontal();
            }
