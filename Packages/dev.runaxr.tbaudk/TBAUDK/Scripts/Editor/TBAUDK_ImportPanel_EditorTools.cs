...
{
    [InitializeOnLoad]
    public class TheBlackArms_ImportPanel_EditorTools : EditorWindow
    {
       private const string Url = "https://github.com/TheBlackArms/TBAUDK/";
        ...
        [MenuItem("TheBlackArms/Import panel/EditorTools", false, 501)]
        public static void OpenImportPanel()
        {
            GetWindow<TheBlackArms_ImportPanel_EditorTools>(true);
        }

        public void OnEnable()
        {
            titleContent = new GUIContent("TheBlackArms Import panel EditorTools");

            TheBlackArms_ImportManager_EditorTools.checkForConfigUpdate();
            LoadJson();
            ...
            _chillHeader = new GUIStyle
            {
                normal =
                {
                    background = Resources.Load("TheBlackArmsUDKHeader.png") as Texture2D,
        ...
        public static void LoadJson()
        {
            ...
            dynamic configJson =
                JObject.Parse(File.ReadAllText(TheBlackArms_Settings.projectConfigPath + TheBlackArms_ImportManager_EditorTools.configName));

            Debug.Log("Server Asset Url is: " + configJson["config"]["serverUrl"]);
            TheBlackArms_ImportManager_EditorTools.serverUrl = configJson["config"]["serverUrl"].ToString();
            ...
            foreach (var asset in assets)
            {
                ...
                else
                {
                    if (GUILayout.Button(
                        ...
                        " " + asset.Key))
                    {
                        TheBlackArms_ImportManager_EditorTools.downloadAndImportAssetFromServer(asset.Value);
                    }

                    if (GUILayout.Button("Del", GUILayout.Width(40)))
                    {
                        TheBlackArms_ImportManager_EditorTools.deleteAsset(asset.Value);
                    }
                }
                GUILayout.EndHorizontal();
            }
